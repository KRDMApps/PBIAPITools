...
export class PBIApiService {
    ...
    clearTable(onNext: (json: any) => void) {
        this.http.delete("api/pbiapi/groups/" + this.groupId + "/datasets/" + this.datasetId + "/tables/" + this.tableName + "/rows").map(response => response.json()).subscribe(onNext, err => console.error(err));
    }
}